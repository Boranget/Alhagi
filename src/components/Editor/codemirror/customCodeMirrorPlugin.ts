...
export function createCustomCodeMirrorPlugin(config: CustomCodeMirrorConfig = {}) {
  const mergedConfig: CodeBlockConfig = {
    ...defaultConfig,
    ...config,
  }
  ...
  const codeBlockViewPlugin = $view(
    ...
    (ctx: Ctx): ((node: Node, view: EditorView, getPos: () => number | undefined) => CustomCodeMirrorBlock) => {
 ...
 * 可以用于合并自定义配置
 */
export function getDefaultCodeMirrorConfig(): CodeBlockConfig {
